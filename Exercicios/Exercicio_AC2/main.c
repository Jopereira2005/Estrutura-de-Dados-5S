...
#include "motor/mapa.h"

void salva_caminho(Pilha *caminho_final, Mapa *labirinto) {
  FILE *arquivo = fopen("dados/saida.txt", "w");
  if (arquivo == NULL) {
  ...
  fclose(arquivo);
  printf("\nCaminho e mapa salvos com sucesso em 'dados/saida.txt'!\n");
}
...
int main() {
  ...
  // Carrega o Mapa
  if (!carrega_mapa(&labirinto, "dados/layout.txt")) {
    return 1;
  }
